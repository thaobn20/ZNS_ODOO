...
class Vefify_Question_Model {
	...
	/**
 * DEBUGGING: Add this method to your Question Model for testing
 */
	public function debug_database() {
    error_log('=== VEFIFY QUIZ DEBUG ===');
    error_log('Questions table: ' . $this->table_prefix . 'questions');
    error_log('Options table: ' . $this->table_prefix . 'question_options');
    
    // Test if tables exist
    $questions_exists = $this->wpdb->get_var("SHOW TABLES LIKE '{$this->table_prefix}questions'");
    $options_exists = $this->wpdb->get_var("SHOW TABLES LIKE '{$this->table_prefix}question_options'");
    
    error_log('Questions table exists: ' . ($questions_exists ? 'YES' : 'NO'));
    error_log('Options table exists: ' . ($options_exists ? 'YES' : 'NO'));
    
    if ($questions_exists) {
        $question_count = $this->wpdb->get_var("SELECT COUNT(*) FROM {$this->table_prefix}questions");
        error_log('Existing questions: ' . $question_count);
    }
