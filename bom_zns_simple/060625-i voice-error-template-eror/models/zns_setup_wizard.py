...
class ZnsSetupWizard(models.TransientModel):
    ...
    def action_step_1_check_connection(self):
        ...
        try:
            ...
            return self._continue_to_step_2()
    ...
    def _continue_to_step_2(self):
        """Continue to step 2: Templates"""
        # Check available templates
        templates = self.env['zns.template'].search([('active', '=', True)])
        self.template_count = len(templates)
        
        if not templates:
            # No templates - offer to sync
            return {
                'type': 'ir.actions.client',
                'tag': 'display_notification',
                'params': {
                    'title': 'No Templates Found',
                    'message': 'No ZNS templates found. Would you like to auto-sync templates from BOM?',
                    'type': 'warning',
                    'sticky': True,
                }
            }
...
🎯 Business Type: {dict(self._fields['business_type'].selection).get(self.business_type, 'Custom')}
